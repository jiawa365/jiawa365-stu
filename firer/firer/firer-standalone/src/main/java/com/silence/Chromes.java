...
//        chromeOptions.addArguments("--user-data-dir=D:\\temp\\chromes\\Chrome\\UserData");
        ...
        FChromeDriver wb = open("D:\\temp\\chromes\\Chrome\\Application\\chrome.exe", "D:\\temp\\chromes\\Chrome\\taobao\\UserData");
        wb.get("https://taojinbi.taobao.com/index.htm");
     ...
     * @param y
     */
    public static void dragAndDrop(WebDriver wb,WebElement element,int x,int y){
        new Actions(wb).dragAndDropBy(element,x,y).perform();
    }

    /**
     * 上传文件，仅适用于[type="file"]节点
     * @param element
     * @param filePath
     */
    public static void upload(WebElement element,String filePath){
        element.sendKeys(new CharSequence[]{filePath});
    }
